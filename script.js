...
function editarSaldoAtual() {
    ...
    // 1. Calcula as Entradas do mês atual
    const totalE = db.entradas.filter(e => {
        const dt = new Date(e.data + "T00:00:00");
        return dt.getMonth() === mesAt && dt.getFullYear() === anoAt;
    }).reduce((acc, e) => acc + e.valor, 0);

    // 2. Calcula as Dívidas PAGAS do mês atual
    const totalD = db.dividas.filter(d => {
        const dt = new Date(d.data + "T00:00:00");
        return dt.getMonth() === mesAt && dt.getFullYear() === anoAt && d.paga === true;
    }).reduce((acc, d) => acc + d.valor, 0);

    // 3. NOVO: Calcula o total de Investimentos (Patrimônio)
    // Subtraímos isso pois o dinheiro saiu da conta corrente para a corretora/reserva
    const totalP = db.patrimonio.reduce((acc, p) => acc + p.valor, 0);

    // 4. Calcula o saldo que o sistema projeta (Entradas - Dívidas Pagas - Investimentos)
    const saldoSistema = totalE - totalD - totalP;

    // 5. Pergunta ao usuário o valor real
    const novoSaldoStr = prompt(`Saldo no sistema: R$ ${saldoSistema.toFixed(2)}\nDigite o saldo real (em conta) que você possui agora:`);
    ...
    // 6. Calcula a diferença e gera o lançamento de ajuste
